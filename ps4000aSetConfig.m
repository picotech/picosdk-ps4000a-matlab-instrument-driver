function [ps4000aStructs, ps4000aEnuminfo] = ps4000aSetConfig()
% PS4000ASETCONFIG Configures paths according to platforms and loads
% information from prototype files for PicoScope 4000 Series (A API)
% Oscilloscopes.
%
% This function allows the information from the prototype files to be
% loaded into the workspace that the picotech_ps4000a_generic.mdd
% Instrument Driver is accessed from.
%
...
% functions from the ps4000a/libps4000a shared library. The folder that
...
% Copyright: © 2019 Pico Technology Ltd. See LICENSE file for terms.
    ...
    if (~any(strcmp(ps4000aConfigInfo.psTbxName, {ps4000aConfigInfo.v.Name})))
        ...
        warning('ps4000aSetConfig:PSTbxNotFound', 'PicoScope Support Toolbox not found, searching for folder.');
        ...
        if (isempty(ps4000aConfigInfo.psTbxFound))

            ps4000aConfigInfo.psTbxNotFoundWarningMsg = sprintf(['Please either:\n'...
            ...
            warning('ps4000aSetConfig:PSTbxDirNotFound', ['PicoScope Support Toolbox not found. ', ps4000aConfigInfo.psTbxNotFoundWarningMsg]);
